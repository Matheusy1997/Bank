...
namespace Bank.Service
{
    internal class BankDelegate
    {
        ...
        {
            {1, (account, acc) => Deposit.PerformDeposit(acc, new ConsoleMessageService())},
            {2, (account, acc) => Withdraw.PerformWithdraw(acc, new ConsoleMessageService())},
            {3, (account, acc) => Loan.PerformLoan(acc, new ConsoleMessageService())},
            {4, (account, acc) => UpdateLoanLimit.PerformUpdateLoanLimit(acc)},
            {5, (account, acc) => Transfer.TransferAccount(account, acc, new ConsoleMessageService())}
        };
        ...
        public static Dictionary<int, SavingsOperation> SavingsOperations = new Dictionary<int, SavingsOperation>
        {
            ...
            {4, (account, acc) => Transfer.TransferAccount(account, acc, new ConsoleMessageService())}
        };

        public static Dictionary<int, MenuOption> MenuOperations = new Dictionary<int, MenuOption>
        {
            {1, (account) => Create.CreateAccount(account)},
